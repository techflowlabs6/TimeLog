      ...
      >
        ...
        <div className="pt-4 border-t border-base-800/80">
          <div
            ...
            className={`flex items-center ${collapsed ? 'justify-center' : 'gap-3'} mb-3 p-2 rounded-xl hover:bg-base-850 cursor-pointer transition-colors group`}
            title="Click to view your performance profile"
          >
            <div className="w-8 h-8 rounded-full bg-accent/20 text-accent group-hover:bg-accent group-hover:text-base-950 transition-colors flex items-center justify-center font-mono text-xs font-bold border border-accent/30 shrink-0 shadow-xs">
              {(profile?.full_name || profile?.email || '?').slice(0, 1).toUpperCase()}
            </div>
            {!collapsed && (
              <div className="min-w-0 flex-1">
                <div className="text-sm font-bold text-slate-900 dark:text-white group-hover:text-accent transition-colors truncate">
                  {profile?.full_name || 'Member'}
                </div>
                <div className="text-[10px] text-slate-500 dark:text-slate-400 font-mono capitalize font-semibold flex items-center gap-1">
                  <span>{isAdmin ? 'admin' : profile?.role || 'member'}</span>
                  <span className="text-[9px] text-accent">• view KPIs</span>
                </div>
              </div>
            )}
          </div>
