...
export default function PersonProjectBarChart({ rows, projects }) {
  ...
  return (
    <div className="card p-4 sm:p-5">
      ...
      {rows.length === 0 ? (
        <div className="h-64 sm:h-72 flex items-center justify-center text-sm text-base-400">
          No entries yet in this range.
        </div>
      ) : (
        <div className="w-full h-72 sm:h-80">
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={rows} margin={{ left: isMobile ? -15 : 0, right: 10, top: 10, bottom: isMobile ? 25 : 5 }}>
              ...
              />
              <Legend
                wrapperStyle={{ fontSize: 11, color: '#c8cdda', paddingTop: 8 }}
                iconType="circle"
              />
              {projects.map((p) => (
